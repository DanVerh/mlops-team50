...
class Model:
    ...
    def train(self, train_dataloader, eval_dataloader, epochs):
        ...
        for epoch in tqdm(range(epochs)):
            ...
            train_loss, _ = self.model.train_step(
                train_dataloader, self.device, self.loss_fn, self.optimizer
            )
            valid_loss, _ = self.model.evaluate(
                eval_dataloader, self.device, self.loss_fn
            )
    ...
    def predict(self, test_seq, test_mask):
        with torch.no_grad():
            return np.argmax(
                self.model(test_seq.to(self.device), test_mask.to(self.device))
                .detach()
                .cpu()
                .numpy(),
                axis=1,
